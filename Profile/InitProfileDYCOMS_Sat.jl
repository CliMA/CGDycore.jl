...
#F[3]=θ-T*(p_d/1.e5)^(-R_d/cp_d)
F[3]=θ-T*(p/1.e5)^(-R_d/cp_d)
#F[3]=θ-T*(p_d/MSLP)^(-R_d/cp_d)
#F[3]=θ-T*(p/MSLP)^(-R_d/cp_d)
#F[4]=θ_l-θ*exp(-FT(LH_v0)/(cp_d*T)*q_l/(1-q_t))
...
println(p," ",z," ",T," ",T," ",rH," ",q_v," ",sqrt(uP(z)^2+vP(z)^2)," ",asind(uP(z)/sqrt(uP(z)^2+vP(z)^2))," ",q_t," ",
  uP(z)," ",vP(z))
dz=z_i/84
for i=1:84
