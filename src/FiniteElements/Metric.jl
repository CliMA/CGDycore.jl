function Metric!(backend,FT,FE::DGElement,Grid,NumberThreadGPU,zS,::Grids.Quad)
  ...
  group = (N,N,M,1,1)
  ndrange = (N,N,M,Nz,NF)
  KMetricKernel1! = MetricQuadKernel1!(backend,group)
  KMetricKernel1!(Metric.dXdxI,Metric.J,Metric.X,FE.DS,FE.DSZ,Val(N),Val(M);ndrange=ndrange)
  KMetricKernel2! = MetricQuadKernel2!(backend,group)
  KMetricKernel2!(Metric.dXdxI,Metric.J,Metric.X,FE.DS,FE.DSZ,Val(N),Val(M);ndrange=ndrange)
  KMetricKernel3! = MetricQuadKernel3!(backend,group)
  KMetricKernel3!(Metric.dXdxI,Metric.J,Metric.X,FE.DS,Val(N),Val(M);ndrange=ndrange)
  group = (DoF,M,1,1)
  ndrange = (DoF,M,Nz,NF)
...
@kernel inbounds = true function MetricQuadKernel3!(dXdxI,J,@Const(X),@Const(DH), ::Val{N}, ::Val{M}) where {N,M}
...
@kernel inbounds = true function MetricQuadKernel2!(dXdxI,J,@Const(X),@Const(DH),@Const(DV), ::Val{N}, ::Val{M}) where {N,M}
...
@kernel inbounds = true function MetricQuadKernel1!(dXdxI,J,@Const(X),@Const(DH),@Const(DV), ::Val{N}, ::Val{M}) where {N,M}
...
function PointsFromGrid(backend,FT,Grid)
  ...
  for iF = 1 : NF
    for iP = 1 : nP  
      iN = Grid.Faces[iF].N[iP]  
      F[iP,1,iF] = Grid.Nodes[iN].P.x
      F[iP,2,iF] = Grid.Nodes[iN].P.y
      F[iP,3,iF] = Grid.Nodes[iN].P.z
    end  
  end  
  copyto!(FGPU,F)
  return FGPU
