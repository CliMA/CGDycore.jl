...
@kernel function LimitKernel!(qMin,qMax,@Const(Rhoq),@Const(Rho),@Const(Glob))
...
# gi, gj, gz, gF = @index(Group, NTuple)
  ...
  if Iz <= Nz
    ...
    @inbounds q[I,J,iz] = median([qMinS[iz], RhoTrColS[I,J,iz] / RhoColS[I,J,iz] +
      l0 * (JJ[ID,1,Iz,IF] + JJ[ID,2,Iz,IF]) * w[I] * w[J] / sumJ[iz],  qMaxS[iz]])
  ...
  if Iz <= Nz
    ...
    if abs(resp[iz]) <= eta 
      ...
      @inbounds qLoc = median([qMinS[iz],  RhoTrColS[I,J,iz] / RhoColS[I,J,iz] + 
        (l0 + dlFD) * (JJ[ID,1,Iz,IF] + JJ[ID,2,Iz,IF]) * w[I] * w[J] / sumJ[iz], qMaxS[iz]])
  ...
  for iTer = 1 : 5
    if Iz <= Nz && conv[iz]
      ID = I + (J - 1) * N  
      @inbounds q[I,J,iz] = median([qMinS[iz], RhoTrColS[I,J,iz] / RhoColS[I,J,iz] +
        lc[iz] * (JJ[ID,1,Iz,IF] + JJ[ID,2,Iz,IF]) * w[I] * w[J] / sumJ[iz],  qMaxS[iz]])
      @inbounds @atomic resc[iz] += (JJ[ID,1,Iz,IF] + JJ[ID,2,Iz,IF]) * w[I] * w[J] / sumJ[iz] *
        (q[I,J,iz] * RhoColS[I,J,iz] - RhoTrColS[I,J,iz])
    end  
    @synchronize
