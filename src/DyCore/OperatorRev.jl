function CurlColumn!(FuC,FvC,Fw,uC,vC,w,RhoC,Fe,dXdxI,ThreadCache)
  ...
  @inbounds for iz = 1 : Nz 
    ...
    @views @. tempwZ[:,:,1,iz] = dXdxI[:,:,1,iz,3,1] * vC[:,:,iz] - dXdxI[:,:,1,iz,3,2] * uC[:,:,iz]
    @views @. tempwZ[:,:,2,iz] = dXdxI[:,:,2,iz,3,1] * vC[:,:,iz] - dXdxI[:,:,2,iz,3,2] * uC[:,:,iz]
  end  
  ...
  @inbounds for iz = 1 : Nz 
    ...
    @views @. temp[:,:,1] = dXdxI[:,:,1,iz,1,1] * vC[:,:,iz] - dXdxI[:,:,1,iz,1,2] * uC[:,:,iz]
    @views @. temp[:,:,2] = dXdxI[:,:,2,iz,1,1] * vC[:,:,iz] - dXdxI[:,:,2,iz,1,2] * uC[:,:,iz]
    ...
    @views @. temp[:,:,1] = dXdxI[:,:,1,iz,2,1] * vC[:,:,iz] - dXdxI[:,:,1,iz,2,2] * uC[:,:,iz]
    @views @. temp[:,:,2] = dXdxI[:,:,2,iz,2,1] * vC[:,:,iz] - dXdxI[:,:,2,iz,2,2] * uC[:,:,iz]
...
#   @views DerivativeZ!(DZpC,pC[:,:,iz],DZ)
    @views @. FuC[:,:,iz] -= RhoC[:,:,iz] * 
      ...
      (dXdxI[:,:,1,iz,1,2] + dXdxI[:,:,2,iz,2,1]) * DYpC)
    @views @. FvC[:,:,iz] -=  RhoC[:,:,iz] *
      ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DXpC + 
...
function DivRhoGrad!(F,cC,RhoC,Fe,dXdxI,J,ThreadCache,Koeff)
  ...
  @inbounds for iz = 1 : Nz
    ...
    @views @. GradDx = 0.5 * RhoC[:,:,iz,] * ((dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * DxcC + 
      (dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DycC)
    @views @. GradDy = 0.5 * RhoC[:,:,iz,] * ((dXdxI[:,:,1,iz,1,2] + dXdxI[:,:,2,iz,1,2]) * DxcC + 
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * DycC)

    @. Div = 0.0
    @views @. temp = 0.5 * ((dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * GradDx + 
      (dXdxI[:,:,1,iz,1,2] + dXdxI[:,:,2,iz,1,2]) * GradDy) 
    DerivativeX!(Div,temp,DW)
    @views @. temp = 0.5 * ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * GradDx + 
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * GradDy)
    DerivativeY!(Div,temp,DW)

    @views @. F[:,:,iz] -= 2.0 * Koeff * Div / (J[:,:,1,iz] + J[:,:,2,iz])
  end    
end    
...
function DivRhoGrad!(F,cC,RhoC,Fe,dXdxI,J,ThreadCache)
  ...
  @inbounds for iz = 1 : Nz
    ...
    @views @. GradDx = 0.5 *((dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * DxcC + 
      (dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DycC)
    @views @. GradDy = 0.5 *((dXdxI[:,:,1,iz,1,2] + dXdxI[:,:,2,iz,1,2]) * DxcC + 
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * DycC)
    ...
    @views @. F[:,:,iz] = 2.0 * Div / (J[:,:,1,iz] + J[:,:,2,iz])
  end    
end    
...
function GradDiv!(FuC,FvC,uC,vC,Fe,dXdxI,J,ThreadCache)
  ...
  @inbounds for iz = 1 : Nz
    @. Div = 0.0  
    @views @. temp = 0.5 * ((dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * uC[:,:,iz] +
      (dXdxI[:,:,1,iz,1,2] + dXdxI[:,:,2,iz,1,2]) * vC[:,:,iz])
    DerivativeX!(Div,temp,D)
    @views @. temp = 0.5 *((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * uC[:,:,iz] + 
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * vC[:,:,iz])
    ...
    @views @. FvC[:,:,iz] = ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DxDiv +
...
function GradDiv!(FuC,FvC,uC,vC,RhoC,Fe,dXdxI,J,ThreadCache,Koeff)
  ...
  @inbounds for iz = 1 : Nz
    ...
    @views @. FuC[:,:,iz] -= Koeff * RhoC[:,:,iz] * ((dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * DxDiv +
      (dXdxI[:,:,1,iz,2,1] +dXdxI[:,:,2,iz,2,1]) * DyDiv) / (J[:,:,1,iz] + J[:,:,2,iz])
    @views @. FvC[:,:,iz] -= Koeff * RhoC[:,:,iz] * ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DxDiv +
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * DyDiv) / (J[:,:,1,iz] + J[:,:,2,iz])
  end  
end  
...
function RotCurl!(FuC,FvC,uC,vC,Fe,dXdxI,J,ThreadCache)
  ...
  @inbounds for iz = 1 : Nz
    ...
    @views @. FuC[:,:,iz] = ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DxW +
...
function RotCurl!(FuC,FvC,uC,vC,RhoC,Fe,dXdxI,J,ThreadCache,Koeff)
  ...
  @inbounds for iz = 1 : Nz
    ...
    @views @. FvC[:,:,iz] -= Koeff * RhoC[:,:,iz] * (-(dXdxI[:,:,1,iz,1,1] + dXdxI[:,:,2,iz,1,1]) * DxW -
      (dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DyW) / (J[:,:,1,iz] + J[:,:,2,iz])
    @views @. FuC[:,:,iz] -= Koeff * RhoC[:,:,iz] * ((dXdxI[:,:,1,iz,2,1] + dXdxI[:,:,2,iz,2,1]) * DxW +
      (dXdxI[:,:,1,iz,2,2] + dXdxI[:,:,2,iz,2,2]) * DyW) / (J[:,:,1,iz] + J[:,:,2,iz])
  end   
end   
