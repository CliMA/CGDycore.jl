...
@inbounds for iz=1:nz  
  @views @. @fastmath vHat[:,:,iz] = dXdxIC[:,:,iz,1,2]*v1CG[:,:,iz] - dXdxIC[:,:,iz,1,1]*v2CG[:,:,iz]
  @views mul!(Vort3[:,:,iz],CG.DS,vHat[:,:,iz])
  @views @. @fastmath vHat[:,:,iz] = dXdxIC[:,:,iz,2,2]*v1CG[:,:,iz] - dXdxIC[:,:,iz,2,1]*v2CG[:,:,iz]
  @views mul!(Temp[:,:,iz],vHat[:,:,iz],CG.DST)
  @views @. Vort3[:,:,iz] += Temp[:,:,iz]
  @views @. @fastmath vHat[:,:,iz] = dXdxIC[:,:,iz,3,2]*v1CG[:,:,iz] - dXdxIC[:,:,iz,3,1]*v2CG[:,:,iz]
end  
if nz>1
  @views @.@fastmath Vort3[:,:,1] += 0.5*(vHat[:,:,2] - vHat[:,:,1])
  @inbounds for iz=2:nz-1
    @views @. @fastmath Vort3[:,:,iz] += 0.25*(vHat[:,:,iz+1] - vHat[:,:,iz])
  end  
  @views @. @fastmath Vort3[:,:,nz] += 0.5*(vHat[:,:,nz] - vHat[:,:,nz-1])
end
...
if Global.Model.Coriolis
  ...
  if str == "Sphere"
      ...
      @inbounds for iz = 1:nz
        @views @. @fastmath Vort3[:,:,iz] -= 2.0*Omega*sin(lat)*JC[:,:,iz]  
      end   
  elseif str == "Beta-Plane"
      ...
      f0 = Global.f0
      y0 = Global.y0
      @views @fastmath Vort3 .= Vort3 .- (f0 .+ beta0 .* (
        reshape(abs.(0.5.*(X[:,:,1,2,:,:] .+ X[:,:,2,2,:,:])),OP,OP,NF,nz) .- y0)) .* JC
  end
end
  @views @. @fastmath FuHat[:,:,:,uPos] -= Vort3[:,:,:] * v2CG[:,:,:] 
  @views @. @fastmath FuHat[:,:,:,vPos] += Vort3[:,:,:] * v1CG[:,:,:] 

  @inbounds for iz=1:nz+1
    # DXa^{11}w
    @views @. @fastmath wHat[:,:,iz] = dXdxIF[:,:,iz,1,1] * wCG[:,:,iz]
  ...
  if nz>1
    @views @. @fastmath DZuuHat31[:,:,1] = 0.5*(vHat[:,:,2] - vHat[:,:,1])
    @inbounds @fastmath for iz=2:nz
      @views @. DZuuHat31[:,:,iz] = 0.5*(vHat[:,:,iz] - vHat[:,:,iz-1])
    end  
    @views @. @fastmath DZuuHat31[:,:,nz+1] = 0.5*(vHat[:,:,nz] - vHat[:,:,nz-1])
  end
  @views @. @fastmath wHat[:,:,:] = dXdxIF[:,:,:,3,1] * wCG[:,:,:]
  @inbounds @fastmath for iz=1:nz
    @views @. DZwwHat31[:,:,iz] = 0.5*(wHat[:,:,iz+1] - wHat[:,:,iz])
  end  
  @views @. @fastmath FuHat[:,:,:,uPos] += 
    DZwwHat31[:,:,:] * wCCG[:,:,:] +
  ...
  @views @. @fastmath DZwwHat31[:,:,:] = DZwwHat31[:,:,:] * v1CG[:,:,:]
  @views @. @fastmath FuHat[:,:,1:nz-1,wPos] +=
    (-DXwHat11[:,:,2:nz] - DYwHat21[:,:,2:nz] + DZuuHat31[:,:,2:nz]) *
    (0.5*(v1CG[:,:,1:nz-1] + v1CG[:,:,2:nz])) -
    0.5*(DZwwHat31[:,:,1:nz-1]+DZwwHat31[:,:,2:nz])

  @inbounds @fastmath for iz=1:nz+1
    # DXa^{12}w
  ...
  @views @. @fastmath vHat[:,:,:] = dXdxIC[:,:,:,3,3] * v2CG[:,:,:]
  if nz>1
    @views @. @fastmath DZuuHat32[:,:,1] = 0.5*(vHat[:,:,2] - vHat[:,:,1])
    @inbounds for iz=2:nz
      @views @. @fastmath DZuuHat32[:,:,iz] = 0.5*(vHat[:,:,iz] - vHat[:,:,iz-1])
    end
    @views @. @fastmath DZuuHat32[:,:,nz+1] = 0.5*(vHat[:,:,nz] - vHat[:,:,nz-1])
  end
  @views @. @fastmath wHat[:,:,:] = dXdxIF[:,:,:,3,2] * wCG[:,:,:]
  @views @. @fastmath DZwwHat32[:,:,:] = 0.5*(wHat[:,:,2:nz+1] - wHat[:,:,1:nz])

  @views @. @fastmath FuHat[:,:,:,vPos] += 
    DZwwHat32[:,:,:] * wCCG[:,:,:] +
  ...
  @views @. @fastmath DZwwHat32[:,:,:] = DZwwHat32[:,:,:] * v2CG[:,:,:]
  @views @. @fastmath FuHat[:,:,1:nz-1,wPos] +=
