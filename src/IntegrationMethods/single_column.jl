...
function spatial_residual!(du, u, ss::Single_Stack, t)
    ...
    @info "spatial residual norm(du) = ", norm(du)
...
P(ρθ) = _MSLP^(-_R_m/_C_v)*_R_d^(_C_p/_C_v)*(ρθ)^(_C_p/_C_v)
dP(ρθ) = _C_p/_C_v * _MSLP^(-_R_m/_C_v)*_R_d^(_C_p/_C_v)*(ρθ)^(_C_p/_C_v - 1)
...
for i = 1:N
    z = (i-0.5)*L/N
    Tvᵢ, pᵢ, ρᵢ = DecayingTemperatureProfile(z; T_virt_surf = 300.0, T_min_ref = 300.0, _R_d = _R_d, _grav = _grav, _MSLP = _MSLP)
    ρ[i]  = ρᵢ
...
CLIMA_TIMESTEPPER = true
N_iter = 1
dt = 100.0

if CLIMA_TIMESTEPPER
    ...
    u = solve(single_column_prob_wfact_split , algorithm; dt=dt)
...
split!(u[:,end], ss.ρ, ss.w, ss.ρθ)
    



zz = Array(LinRange(Δz0/2, L - Δz0/2, N))
...
ax1.plot(ss.ρ, zz, "-", fillstyle = "none", label = "T=$(dt*N_iter)")
ax1.legend()
ax1.set_ylabel("ρ")

ax2.plot(w, zz_h, "-o", fillstyle = "none", label = "T=0")
ax2.plot(ss.w, zz_h,  "-", fillstyle = "none", label = "T=$(dt*N_iter)")
ax2.legend()
ax2.set_ylabel("w")

ax3.plot(ρθ, zz,  "-o", fillstyle = "none", label = "T=0")
ax3.plot(ss.ρθ, zz, "-", fillstyle = "none", label = "T=$(dt*N_iter)")
ax3.legend()
ax3.set_ylabel("ρθ")
