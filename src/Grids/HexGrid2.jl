...
function generate_hex_grid(num_rings::Int; r::Float64=1.0)
    ...
    # 1. Generate Cell Centers
    cell_centers = Tuple{Int, Int, Int}[]
    push!(cell_centers, (0, 0, 0)) # Ring 0 (center)
    ...
    # Edges: Boundary if touching only 1 cell
    ...
    # Nodes: Interior if shared by 3 cells (is_boundary = false)
    #        Boundary if shared by fewer than 3 cells (is_boundary = true)
...
# --- Run Exports ---
grid = generate_hex_grid(2) # 2 rings = 19 hexagons
