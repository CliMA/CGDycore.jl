...
function JacobiCartDG2GPU!(X,dXdx,dXdxI,J,Rotate,FE,F,Rad)
  ...
  KJacobiCartDG2Kernel! = JacobiCartDG2Kernel!(backend,group)

  KJacobiCartDG2Kernel!(X,dXdx,dXdxI,J,Rotate,FE.xw,F,Rad,ndrange=ndrange)
end

@kernel inbounds = true function JacobiCartDG2Kernel!(X,dXdx,dXdxI,JJ,Rotate,@Const(ksi),@Const(F),Rad)
  ...
  if IF <= NF
    ...
    C = @SArray([eltype(X)(-1)+ksi2  eltype(X)(-1)+ksi1
                 ...
                 eltype(X)(1)+ksi2   eltype(X)(1)+ksi1
                 eltype(X)(-1)-ksi2   eltype(X)(1)-ksi1])
    @views dXdxLoc[I,J,:,1:2,iF] .= eltype(X)(0.25) * B * C
    @views @. dXdx[:,1:2,1,ID,1,IF] = dXdxLoc[I,J,:,1:2,iF]
    ...
    Rotate[1,1,1,ID,1,IF] = eltype(X)(1)
    Rotate[2,1,1,ID,1,IF] = eltype(X)(0)

    Rotate[1,2,1,ID,1,IF] = eltype(X)(0)
    Rotate[2,2,1,ID,1,IF] = eltype(X)(1)

    Rotate[1,3,1,ID,1,IF] = eltype(X)(0)
    Rotate[2,3,1,ID,1,IF] = eltype(X)(0)
  end  
end  
