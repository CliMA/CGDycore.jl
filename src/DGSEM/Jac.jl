...
@kernel inbounds=true function SchurBoundaryKernel!(SchurD,SchurL,SchurU,@Const(A12_11),@Const(A22_diag), 
  ...
  if ID <= NumI
    ...
    @inbounds for col in 1:3
      ...
      @atomic :monotonic SchurD[1, col, iz, ID] -= p1
      @atomic :monotonic SchurD[2, col, iz, ID] -= p2 + pg          # Cgrav (ϱw_M ← interior ϱ)
      @atomic :monotonic SchurD[3, col, iz, ID] -= p3
      # C_p → SchurU[:,col,iz-1]  (node-1 response)
      if !is_iz1
        r2_1 = r2[1,col];  r3_1 = r3[1,col]
        SchurU[1, col, iz-1, ID] -= c2p1*r2_1 + c3p1*r3_1
    ...
    if !is_iz1
      ...
      @unroll for (slot, col) in ((1, 2), (2, 3))   # slot1→col2 (ϱw_M), slot2→col3 (ϱθ_M)
        ...
        # C_p → SchurD[:,col,iz-1] self correction (node-1 response)
        r2_1 = r2[1,slot];  r3_1 = r3[1,slot]
        @atomic :monotonic SchurD[1, col, iz-1, ID] -= c2p1*r2_1 + c3p1*r3_1
        @atomic :monotonic SchurD[2, col, iz-1, ID] -= c2p2*r2_1 + c3p2*r3_1
        @atomic :monotonic SchurD[3, col, iz-1, ID] -= c2p3*r2_1 + c3p3*r3_1
        # C_self → SchurL[:,col,iz-1]  (full interior(iz) response + Cgrav)
        p1 = zero(FT);  p2 = zero(FT);  p3 = zero(FT);  pg = zero(FT)
...
function ldivVerticalS!(F::JacobianCacheMarcoSplitNS, NumberThreadGPU::Int64) 
  nz = F.nz
  NumI = F.NumI
...
function ldivVerticalF!(cache_jacobian::JacobianCacheMarcoSplitNS, b, dz, NumberThreadGPU)
  ...
  KldivVerticalFKernel! = ldivVerticalFKernel!(backend,group)
  KldivVerticalFKernel!(rs,b,A12_11,A22_diag,DWS,Th_cache,dpdRhoTh_cache,Gblk,SA,dz,nz,fac,invwB, 
    Val(M),ndrange=ndrange)
end  

@kernel inbounds = true function ldivVerticalFKernel!(rs,@Const(b),@Const(A12_11),@Const(A22_diag),@Const(DWS),
  @Const(Th_cache),@Const(dpdRhoTh_cache),@Const(Gblk),@Const(SA),@Const(dz),nz,fac,invwB, ::Val{M}) where M
...
function ldivVerticalB!(cache_jacobian::JacobianCacheMarcoSplitNS, b, dz, NumberThreadGPU)
  ...
  KldivVerticalBKernel! = ldivVerticalBKernel!(backend,group)
  KldivVerticalBKernel!(b,rs,A12_11,A22_diag,DWS,Th_cache,dpdRhoTh_cache,Gblk,SA,dz,nz,fac,invwB, 
    Val(M),ndrange=ndrange)
end  
...
function JacM!(U,fac,DG,Metric,Phys,Cache,JCache::JacobianCacheMarcoSplitNS,Global,VelForm)
  @show "Seriell"
  @views Geo = Cache.Aux[:,:,:,2]
  precompute_gravity!(Geo,Metric.dz,JCache)
  Invfac = eltype(U)(1) / fac
  dz = Metric.dz
  FillJacDGVert!(JCache, U, Metric.dz, Invfac,Phys)
...
function solve_jacobian!(b, cache_jacobian::JacobianCacheMarcoSplitNS, Metric)
  @show "Seriell"
  @show "b",sum(abs.(b)),sum(abs.(cache_jacobian.SA)),cache_jacobian.fac
  ldivVerticalF!(cache_jacobian, b, Metric.dz)
  @show "bF",sum(abs.(b))
  ldivVerticalS!(cache_jacobian,cache_jacobian.rs)
  @show "bS",sum(abs.(b)),sum(abs.(cache_jacobian.rs))
  ldivVerticalB!(cache_jacobian, b, Metric.dz)
  @show "bB",sum(abs.(b))
  stop
end
