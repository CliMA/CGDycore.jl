...
@kernel inbounds = true function ldivVerticalBKernel!(@Const(A13),@Const(A23),@Const(A32),
  ...
  if ID <= DoF
    ...
    r1[1] -= B1_1[iz,ID] * rs[sh + 1]
    r1[M] -= B1_4[iz,ID] * rs[sh + 4]
    @unroll for i = 1 : M
      r1[i] -= B1_23[i,1,iz,ID] * rs[sh + 2] + B1_23[i,2,iz,ID] * rs[sh + 3]
    end
    @unroll for i = 1 : M - 2
      r2[i] -= B2_23[i,1,iz,ID] * rs[sh + 2] + B2_23[i,2,iz,ID] * rs[sh + 3]
      r3[i] -= B3_14[i,1,iz,ID] * rs[sh + 1] + B3_14[i,2,iz,ID] * rs[sh + 4]
    end
    if iz > 1
      r1[1] -= B1m_34[1,iz,ID] * rs[sh-1] + B1m_34[2,iz,ID] * rs[sh]
    end
    if iz < nz
      r1[M] -= B1p_12[1,iz,ID] * rs[sh+5] + B1p_12[2,iz,ID] * rs[sh+6]
    end
    @unroll for i = 1 : M - 2
      r3[i] += -invfac * FacGrav * r1[i+1]
      @unroll for j = 1 : M - 2
...
@kernel inbounds = true function FillJacDGVertKernel!(A13,A23,A32,B1m_34,B1_1,B1_23,B1_4,
  ...
  if ID <= DoF
    ...
    @unroll for i = 1 : M - 2
      @unroll for j = 1 : M - 2
        SA[i,j,iz,ID] = eltype(U)(0)
        @unroll for k = 1 : M - 2
          SA[i,j,iz,ID] += A32[i,k,iz,ID] * A23[k,j,iz,ID]
        end
        if i == j
          SA[i,j,iz,ID] = fac - (FacGrav / fac) * A13[i+1,j,iz,ID] -
            (eltype(U)(1) / fac) * SA[i,j,iz,ID]
        else
