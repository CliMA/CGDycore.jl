function gamma(x) 
    p[1]  = 0.999999999999999990e+00;
    p[2]  = -0.422784335098466784e+00;
    p[3]  = -0.233093736421782878e+00;
    p[4]  = 0.191091101387638410e+00;
    p[5]  = -0.024552490005641278e+00;
    p[6]  = -0.017645244547851414e+00;
    p[7]  = 0.008023273027855346e+00;
    p[8]  = -0.000804329819255744e+00;
    p[9]  = -0.000360837876648255e+00;
    p[10]  = 0.000145596568617526e+00;
    p[11] = -0.000017545539395205e+00;
    p[12] = -0.000002591225267689e+00;
    p[13] = 0.000001337767384067e+00;
    p[14] = -0.000000199542863674e+00;
    n     = round(x - 2);
    w     = x - (n + 2);
    y = ((((((((((((p[13] * w + p[12]) * w + p[11]) * w + p[10]) * w + p[9]) * w + p[8]) * w + p[7]) * w + p[6]) * w +
             p[5]) *
                w +
            p[4]) *
               w +
        ...
        if (StructuredGrid(grid) && (coordinates_ == Coordinates::XY)) {
            ...
            if (atlas_omp_get_max_threads() > 1) {
                atlas_omp_parallel {
                    ...
                    const idx_t begin = static_cast<idx_t>(thread_num * size_t(structured_grid.size()) / num_threads);
                    const idx_t end =
