function fVel(x,time,Global,Param)
    ...
    elseif ProfVel == "Divergent"  
      ...
      lonP = Lon - 2.0e0 * pi * time / EndTime
      uS = 10.0 / EndTime * sin(lonP)^2 * 
        sin(2.0 *Lat) * cos(pi * time / EndTime) + 2.0e0 * pi / EndTime * cos(Lat)
      vS = 10.0 / EndTime * sin(2.0 * lonP) * cos(Lat) * cos(pi * time / EndTime)
    ...
    elseif ProfVel == "AdvectionSchaer"  
      ...
      uS = Param.uMax      
    elseif ProfVel == "SchaerSphere"
      (Lon,Lat,R) = cart2sphere(x[1],x[2],x[3])
      uS = Param.uEq * cos(Lat)
      vS = 0.0
    elseif ProfVel == "hyperdiffcart"
    ...
    elseif ProfVel == "barowavecart"
      ...
      vS=0
    elseif ProfVel == "baldauf"
      uS=Param.uMax
      vS=0
    elseif ProfVel == "barowavedrysphere"
  #     # parameters

    elseif ProfVel == "gravityhill"
      uS=Param.uMax
      vS=0
