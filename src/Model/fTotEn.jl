function fTotEn(x,time,Global,Param)
  Model=Global.Model
  Phys=Global.Phys
  ...
  elseif str == "barowavesphere"
    ...
    E = Th*(Pressure/Phys.p0)^Phys.kappa*Phys.Cvd + Phys.Grav * Z + 0.5 * uS * uS
  ...
  elseif str == "warmbubble2dx"
    ...
    pLoc=Phys.p0*(1-Phys.Grav*x3*Phys.kappa/(Phys.Rd*Th0))^(1.0/Phys.kappa)
    ThLoc=Th0;
    if rr<rC0
      ThLoc=ThLoc+DeltaTh*cos(0.5*pi*rr/rC0)^2;
    end
    E = ThLoc*(pLoc/Phys.p0)^Phys.kappa*Phys.Cvd + Phys.Grav * x3
