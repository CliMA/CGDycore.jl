mutable struct JacDGVertStruct{FT<:AbstractFloat,
                         AT2<:AbstractArray,
                         AT3<:AbstractArray,
                         AT4<:AbstractArray}
  M::Int
...
function JacDGVert(backend,FT,M,nz,NumG) 
  M2 = M - 2
  fac = 0
  FacGrav = 0
...
#   @show "v2N",rs[sh+1:sh+4]
#   @show r3
    for j = 1 : M - 2
      rs[sh + 1,ID] += -C14_3[1,j,iz,ID] * r3[j]
      rs[sh + 4,ID] += -C14_3[2,j,iz,ID] * r3[j]
      if ID == 1
#     @show rs[sh + 4,ID],C14_3[2,j,iz,ID]
      end
      rs[sh + 2,ID] += -C23_2[1,j,iz,ID] * r2[j]
      rs[sh + 3,ID] += -C23_2[2,j,iz,ID] * r2[j]
    end
#   @show "v3N",rs[sh+1:sh+4]
    rs[sh + 2,ID] += -FacGrav * r11
    rs[sh + 3,ID] += -FacGrav * r1M
#   @show "v4N",rs[sh+1:sh+4]
    if ID == 1
#   @show iz,ID,rs[1:4,1]
    end
    if iz == nz && ID == 1
...
@kernel inbounds = true function FillJacDGVertKernel!(A13,A23,A32,B1m_34,B1_1,B1_23,B1_4,
  ...
  if ID <= DoF
    ...
    if iz < nz
      ...
      B1p_12[1,iz,ID] = -dpdRhoThP * invcS / (wB * dz[iz+1,ID])
      B1p_12[2,iz,ID] = eltype(U)(1) / (wB * dz[iz+1,ID])
    end
    ...
    if iz > 1
      ...
      @atomic :monotonic SchurBand[5,sh-1,ID] += -Th[1] * dpdRhoThM * invcS / dz[iz-1,ID] / wB
      @atomic :monotonic SchurBand[3,sh,ID] += -ThM * dpdRhoTh[1] * invcS * invdz / wB
      @atomic :monotonic SchurBand[4,sh,ID] += Th[1] * dpdRhoTh[1] * invcS * invdz  / wB
      @atomic :monotonic SchurBand[4,sh-2,ID] += cS / dz[iz-1,ID] / wB
      @atomic :monotonic SchurBand[7,sh-2,ID] += -cS / dz[iz-1,ID] / wB
      @atomic :monotonic SchurBand[1,sh+1,ID] += -cS * invdz / wB
...
# @show sum(abs.(JacVert.rs[:,1]))
# @show sum(abs.(JacVert.rs[:,2]))
# @show sum(abs.(JacVert.rs[:,3]))
...
function JacDG(U,Aux,DG,fac,dSdS,dSdM,dMdS,dMdM,z,Phys)
  RowInd = Int[]
  ColInd = Int[]
  Val = Float64[]
  D = DG.DWZ
  ...
  for ID = 1 : DG.NumI
    ...
    Jac = [sparse(fac*I,N,N) -diagz * dSdM              -diagz* dSdS * diagm(dpdRhoTh)
           -diagz * diagm(Geo) * DGeo  sparse(fac*I,N,N) - diagz * dMdM -diagz* dMdS * diagm(dpdRhoTh)
...
function JacDGT(U,Aux,DG,fac,dSdS,dSdM,dMdS,dMdM,z,Phys)
  RowInd = Int[]
  ColInd = Int[]
  Val = Float64[]
  D = DG.DWZ
...
function JacDGTNeu(U,Aux,DG,fac,dSdS,dSdM,dMdS,dMdM,z,Phys)
  RowInd = Int[]
  ColInd = Int[]
  Val = Float64[]
  D = DG.DWZ
  ...
  JacLU = Array{SparseArrays.UMFPACK.UmfpackLU}(undef,size(U,3))
    ...
    Jac = [sparse(fac*I,N,N)  -diagz* dSdS * diagm(dpdRhoTh) -diagz * dSdM
           spzeros(N,N) sparse(fac*I,N,N) - diagz*diagm(Th)*dSdS*diagm(dpdRhoTh)  -diagz*dSdM*diagm(Th)
           -diagz * diagm(Geo) * DGeo -diagz*dMdS*diagm(dpdRhoTh) sparse(fac*I,N,N)-diagz*dMdM]
    JacLU[ID] = lu(Jac)
  return JacLU,Jac
end
