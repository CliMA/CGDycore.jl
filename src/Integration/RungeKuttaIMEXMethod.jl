mutable struct RungeKuttaIMEXMethod{FT<:AbstractFloat}
  nStage::Int
  Type::String
  ARKE::Array{FT, 2}
  bRKE::Array{FT, 1}
...
function RungeKuttaIMEXMethod{FT}() where FT<:AbstractFloat
  nStage=0
  Type=""
  ARKE=zeros(FT,0,0)
  bRKE=zeros(FT,0)
  ...
  bRKI=zeros(FT,0)
  gRKI=zeros(FT,0)
  return RungeKuttaIMEXMethod{FT}(
  nStage,
  Type,
  ARKE,
  bRKE,
...
function RungeKuttaIMEXMethod{FT}(Method) where FT<:AbstractFloat
  str = Method
if str == "ARK32"
    RK.nStage=3;
