...
function CacheAuxStruct(backend,FT,DG,M,nz,NumAux)
  NumG = DG.NumG
  NumI = DG.NumI
...
function CacheStruct(backend,FT,DG::FiniteElements.DGElement,M,nz,NumV,NumStages)
  NumG = DG.NumG
  NumI = DG.NumI
...
function TimeStepperDG(U,Fcn,Jac,dt,nIter,nPrint,DG,Exchange,Metric,Trans,Phys,Param,Grid,
  ...
  CacheAux = CacheAuxStruct(backend,FT,DG,M,nz,NumAux)
  Aux = CacheAux.Aux
  DGSEM.GeoPot(Aux,DG,Metric,Exchange,Global)
  if IntMethod == "Rosenbrock" || IntMethod == "RosenbrockSSP" || IntMethod == "RosenbrockAMD"
    Cache = CacheStruct(backend,FT,DG,M,nz,NumV,TimeStepper.ROS.nStage)
    CacheU = Cache.Vn
  elseif IntMethod == "RungeKutta"
    TimeStepper.RK=RungeKuttaMethod{FT}(Table)
  elseif IntMethod == "IMEX"
  ...
  @time begin
    @inbounds for i = 1 : nIter
      Δt = @elapsed begin
        Rosenbrock!(U,dt,Fcn,CacheAux,Jac,DG,Metric,Phys,Cache,JCache,Exchange,
