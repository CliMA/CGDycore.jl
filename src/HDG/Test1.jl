...
# ==============================================================================
# 4. THERMODYNAMICS & CORRECTED LMARS RIEMANN FLUX
# ==============================================================================
...
"""
Computes the outward LMARS flux F_hat across a face with outward unit normal n (+1 or -1).
U_int: interior state
U_ext: exterior state
"""
function lmars_outward_flux(U_int, U_ext, n)
    ...
    # Normal velocity components along outward normal
    u_i_n = (m_i / rho_i) * n
    u_e_n = (m_e / rho_e) * n
    ...
    # Acoustic interface values along outward normal
    p_hat   = 0.5 * (p_i + p_e) - 0.5 * a_avg * rho_avg * (u_e_n - u_i_n)
    u_hat_n = 0.5 * (u_i_n + u_e_n) - 0.5 / (a_avg * rho_avg) * (p_e - p_i)
    
    # Upwind state selection based on outward velocity direction
    if u_hat_n >= 0.0
        # Flow leaving element
        return [rho_i * u_hat_n * n, (rho_i * u_hat_n * (m_i / rho_i) + p_hat) * n, Theta_i * u_hat_n * n]
    else
        # Flow entering element
        return [rho_e * u_hat_n * n, (rho_e * u_hat_n * (m_e / rho_e) + p_hat) * n, Theta_e * u_hat_n * n]
    end
end
...
function compute_rhs(U)
    ...
    for e in 1:Ne
        ...
        # Left boundary face (i = 1, outward normal n = -1)
        if e == 1
            # Free-slip wall: mirror momentum (u_ext = -u_int), preserve scalar fields
        ...
        # Right boundary face (i = Np, outward normal n = +1)
        if e == Ne
            # Free-slip wall: mirror momentum (u_ext = -u_int), preserve scalar fields
        ...
        # Weak DG Surface assembly
        for var in 1:3
            vol_term = -dF_vol[:, var] + S_vol[:, var]
            
            # Physical flux along outward normal at left (-1) and right (+1) faces
...
dt = 0.04 * dx_min / c_sound_max  # Courant stability limit for explicit GLL acoustic waves
t_final = 1.0
