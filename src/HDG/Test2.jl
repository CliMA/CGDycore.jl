...
# ==============================================================================
# 3. MESH & ISOTHERMAL HYDROSTATIC INITIAL STATE
# ==============================================================================
...
for e in 1:Ne
    for i in 1:Np
        ...
        # Exact Isothermal Hydrostatic Profile
...
@inline function pressure(rho, Theta)
    # Direct ideal gas relation without max/threshold guards
    return p0 * (R_d * Theta / p0)^gamma
end

@inline function speed_of_sound(rho, Theta)
    # Direct acoustic speed without max/threshold guards
#   return sqrt(gamma * pressure(rho, Theta) / rho)
    return 340.0
end

"""
Outward LMARS numerical flux along normal n (+1 or -1)
"""
...
function compute_rhs(U)
    ...
    for e in 1:Ne
        ...
        # Volume derivative (Strong form operator: D_ref * F / J)
        dF_vol = (D_ref * F_vol) ./ J
        
        # Pointwise Gravity Source
        S_vol = zeros(Np, 3)
        for i in 1:Np
            S_vol[i, 2] = -rho_e[i] * g
        end
        ...
        if e == 1
            ...
            @show e,F_hat_L,340*m_e[1]
            @show pressure(rho_e[1],  Theta_e[1])
        else
        ...
        # Weak DG Assembly: dU/dt = -dF/dz + S - M_inv * Surface_Fluxes
        for var in 1:3
            ...
            if e == 1 && var == 2
              @show F_phys_L_n,F_hat_L[var]  
              @show surf_L[1],U[1,1,var]
            end  
