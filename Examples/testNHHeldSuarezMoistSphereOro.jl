...
if Parallel
  ...
  Exchange = CGDycore.InitExchangeCG(Grid,OrdPoly,CellToProc,Proc,ProcNumber,Parallel)
  Output=CGDycore.Output(Topography)
...
# Print initial conditions
  ...
  SumU0 = CGDycore.GlobalSum3D(U,CG)
  if Proc == 1
    @show 0,SumU0
  end
# Statistics
  ...
  elseif IntMethod == "IMEX"
    @time begin
      for i=1:nIter
        ...
        SumU = CGDycore.GlobalSum3D(U,CG)
        if Proc == 1
          @show i,SumU
          percent = i/nIter*100
          @info "Iteration: $i took $Δt, $percent% complete"
