...
JCacheO = DGSEM.JacSplitDGVert(backend,FTB,M,nz,DG)
NumberThreadGPU = 256
DGSEM.precompute_gravity!(Geo,dz,DG.DWZ,JCacheO,NumberThreadGPU)
DGSEM.FillJacSplitDGVert!(JCacheO,U,DG,dz,fac,Phys)
DGSEM.SchurBoundary!(JCacheO)
b = ones(M,nz,DG.NumG,5)
DGSEM.Solve!(JCacheO,b)

JacLU, Jac = JacDGTNeu(U,Aux,DG,fac,dSdS,dSdM,dMdS,dMdM,dGeo,dz,Phys)
c = ones(M*nz*3)
...
bP1 = bP[1:n11]
bP2 = bP[n11+1:end]
L = [JacP11 spzeros(n11,n22)
     JacP21 sparse(I,n22,n22)]
xP = L \ bP     
xP2 = SchurJacP \ xP[n11+1:end]
stop
