...
GridSpherical = Grids.SphericalGrid(backend,FTB,nLon,nLat,LatB,Grids.OrientFaceSphere,Rad,nz)

@show GridSpherical.NumFaces
vtkSkeletonMeshSpherical = Outputs.vtkStruct{Float64}(backend,GridSpherical)
c = ones(FTB,GridSpherical.NumFaces)
for i = 1 : GridSpherical.NumFaces
  c[i] = i
end  
Outputs.vtkSkeleton(vtkSkeletonMeshSpherical,"Spherical", 1, 1,c)
